...
public class Server {
    public static void main (String argv []) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            c = DriverManager
                    .getConnection("jdbc:postgresql://postgresql-yzli.alwaysdata.net:NUMERO PORT/NOM DE LA BASSE DE DONNEE", "IDENTIFIANT", "MOT DE PASSE");

            System.out.println("=> Ouverture de la Base de donn�e");
            ...
            while ((chaine_entree = flux_entree.readLine()) != null) {
                ...
                if (chaine_entree.equals("q") || chaine_entree.length() != 11 || chaine_entree.contains(" ")) {
                    ...
                    System.out.println("Vous avez envoy� ID: " + chaine_entree);
                    ...
                    if (id_exist){
                        if (bool_manger == false){
                            if (solde >= paye) {   // Si il a acces d'argent pour son 2�me repas de la journ�e
                                //Facturation de 3.30�
                                stmt = c.createStatement();
                                String req_pay = "UPDATE client SET solde = solde-"+paye+" WHERE id_client='" + chaine_entree + "'";
                                stmt.executeUpdate(req_pay);
                                stmt.close();
                                System.out.println("ID : "+chaine_entree+" a �tais factur� de"+paye);
